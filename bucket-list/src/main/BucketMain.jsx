...
class BucketMain extends Component {
   ...
   * 사용할 수 있고
   * 여기에서 bucketList의 b_id값의 PK값을 만들기 위해 사용한다.
   */
  id = 1;
   ...
   * 화면을 rendering 한다.
   * 이러한 Mehtod들을 Life Cycle Method라고 한다.
   */

  /**
   * 화면이 이미 완성된 상태에서 데이터가 변화 했을 때 rendering하려고 할 때 실행되는 method
   * insert, update, delete 되었을 때 실행될 메서드
   * 실제 DB와 연동을 하면 이 method에서 AJAX로 서버에 데이터를 전송하고 서버에서는 CRUD를 수행한다.
   * 이 메서드가 React에 의해 호출될 때 State 변수의 변화를 감지할 수 있다.
   * State 변수에 변화가 있어서 화면이 rendering 될때 변화되기 이전 데이터가 pre** 매개변수에 담겨서 전달이 된다.
   * 이 데이터와 현재 데이터를 비교하여 달라졌을 경우 DB에 저장하거나 하는 일을 수행하여 효율을 높일 수 있다.
   */
  ...
  bucketInsert = (bucket_title) => {
    ...
    /**
     * 일반적인 JS코드에서 bucketList.push(bucket)
     * react의 선언적 철학
